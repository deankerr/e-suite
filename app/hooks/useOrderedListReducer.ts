...
export function useOrderedListReducer<T>(initialItems: T[] = []) {
  const [items, dispatch] = useReducer(orderedListReducer<T>, initialItems)

  const updateItems = useCallback((action: Action<T>) => {
    dispatch(action)
  }, [])
