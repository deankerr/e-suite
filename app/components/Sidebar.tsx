...
export const Sidebar = ({ children, className, side }: SidebarProps) => {
  return (
    <div
      className={cn(
        'relative z-20 h-full w-96 overflow-x-hidden overflow-y-hidden border-gray-6 bg-background transition-all duration-300',
        variant[side],
      )}
    >
      <div className={cn('flex h-full flex-col justify-center gap-5 py-6', className)}>
        {children}
      </div>
    </div>
  )
