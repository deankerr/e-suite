...
export const ImageModelCard = ({
  ...
  return (
    <Card {...props} className={cn('flex h-64 w-40 shrink-0 flex-col', className)}>
      ...
      <div className={cn('flex-between absolute inset-x-2 top-2 gap-2 rounded')}>
      ...
      <Inset
        side="bottom"
        className="flex-col-center absolute inset-x-1 bottom-2 min-h-16 space-y-1 bg-black/75 px-3 py-3 text-center"
      >
...
export const ImageModelCardSkeleton = () => {
  return <SkeletonPulse className="h-64 w-40 shrink-0 animate-pulse" />
}
...
export const ImageModelCardHSkeleton = () => {
  return (
    <SkeletonPulse className="mx-auto h-32 w-full max-w-80 shrink-0 animate-pulse border border-grayA-4">
      <SkeletonPulse className="absolute inset-y-0 right-0 h-full w-32 animate-none border-l border-grayA-3" />
    </SkeletonPulse>
  )
}
