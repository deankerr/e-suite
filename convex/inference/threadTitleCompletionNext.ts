  ...
  handler: async (ctx, args) => {
    try {
      const { job, threadId, messages } = await ctx.runMutation(
        internal.inference.threadTitleCompletionNext.start,
        args,
      )
      ...
      console.log('[threadTitleCompletionNext] [input] [anthropic]', message)
      const response = await generateText({
        model: anthropic('claude-3-haiku-20240307'),
        prompt: message,
        maxTokens: 1024,
      })
      console.log('[threadTitleCompletionNext] [output] [anthropic]', response)
      ...
      await ctx.runMutation(internal.inference.threadTitleCompletionNext.complete, {
        jobId: job._id,
        threadId,
        title,
      })
